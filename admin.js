...
const APPEARANCE_KEYS = new Set(
  Object.keys(DEFAULT_APPEARANCE)
);
...
const state = {
  ...
  // Rascunho do editor visual. Enquanto houver alterações não publicadas,
  // a sincronização automática jamais poderá sobrescrever state.draft.
...
async function saveSettings(
  ...
  try {
    ...
    if (appearanceSave) {
      // Somente o botão "Publicar alterações" encerra o rascunho visual.
      state.draft = clone(state.settings);
      state.appearanceDirty = false;
      resetHistory();
    } else if (!state.appearanceDirty) {
      // Sem edição visual pendente, podemos manter o rascunho espelhado no servidor.
      state.draft = clone(state.settings);
      resetHistory();
    } else {
      // Outra área do painel foi salva enquanto o Editor Visual tinha alterações.
      // Atualize apenas as chaves que não pertencem ao editor visual, preservando o rascunho.
      const nextDraft = clone(state.draft);

      Object.entries(settings || {}).forEach(([key, value]) => {
        if (!APPEARANCE_KEYS.has(key)) {
          nextDraft[key] = clone(value);
        }
      });
...
const colorFields = [
  ...
  ['#colorText', 'textColor'],
  ['#colorMuted', 'mutedColor'],
  ['#colorButton', 'buttonColor'],
  ['#colorButtonHover', 'buttonHoverColor'],
  ['#colorBorder', 'borderColor'],
  ['#colorPrice', 'priceColor']
];
...
function updateFocusButtons() {
  $$('.focus-btn').forEach((button) => {
    const target = button.dataset.focus;
    const label = focusLabels[target] || 'elemento';
    const active = state.activeFocus === target;

    button.classList.toggle('is-focused', active);
    button.setAttribute('aria-pressed', String(active));
    button.textContent = active
      ? `🔎 Tirar foco: ${label}`
      : `🔍 Focar elemento: ${label}`;
  });
}
...
async function syncAdminNow() {
  ...
  // Enquanto o Editor Visual tiver alterações não publicadas, não sincronize
  // settings nem rode um ciclo que possa interferir na edição em andamento.
  const appearanceOpen =
    $('#appearance')?.classList.contains('active-page');

  if (appearanceOpen && state.appearanceDirty) {
    return;
  }
  ...
  try {
    ...
    if (sync.settings && !state.appearanceDirty) {
      state.settings = {
        ...DEFAULT_APPEARANCE,
        ...sync.settings
      };
